...
Operating notes for AI agents working in a repository guarded by Strix.

## The one rule

An ungated HEAD does not push. Before any `git push`:

1. Run `strix gate <repoDir>` **as its own command** and confirm every gate is green.
...
3. After a green gate, optionally deliver evidence: `strix emit <repoDir> | fukuro import`.
...
Prefer the repo's own `.strix.json` (`{"gates": [...]`). Without it, Strix derives gates
from package.json scripts (typecheck / lint / build / test). If the automatic detection
runs the wrong thing (e.g. a test script that needs arguments), add `.strix.json` — do not
work around it in your own shell.

## Setup from a fresh clone

```
node cli/strix.ts    # help
node test/smoke.mjs # verify (creates a throwaway git repo under test/.tmp)
strix hooks          # print the settings.json wiring for Claude Code
```
